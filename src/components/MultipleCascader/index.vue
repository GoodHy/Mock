<template>
    <span class="dropTreeLists">
        ...
        <multiCascader :options="configOptions" @on-selected="getSelected" :inputValue="configTips"></multiCascader>
    </span>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getSelected(val) {
      ...
      //已选择选项
      let _configTips = this.configTips;
      console.log(val);
      ...
      if (strnum !== this.commonLength) {
        let _options = this.configOptions;
        if (val.length > 0) {
          this.configTips = this.getSelectVal(_options, val);
        } else {
          this.configTips = "请选择基准";
        }
        console.info(this.configTips, "_selectValue=======>>");
    ...
    getSelectVal(options, val) {
      let _options = options || [];
      let _selectValue = "";
      _options.map((item, index) => {
        if (val.includes(item.value)) {
          _selectValue += item.label + "  ";
        }
        if (item.children) {
          _selectValue += this.getSelectVal(item.children, val);
        }
        return item;
      });
      return _selectValue;
